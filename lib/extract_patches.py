...
def paint_border_overlap(full_imgs, patch_h, patch_w, stride_h, stride_w):
    ...
    if (leftover_h != 0):  #change dimension of img_h
        ...
        tmp_full_imgs = np.zeros((full_imgs.shape[0],img_h+(stride_h-leftover_h),img_w,full_imgs.shape[-1]))
    ...
    if (leftover_w != 0):   #change dimension of img_w
        ...
        tmp_full_imgs = np.zeros((full_imgs.shape[0],full_imgs.shape[1],img_w+(stride_w - leftover_w),full_imgs.shape[-1]))
...
def recompone(data,N_h,N_w):
    ...
    full_recomp = np.empty((N_full_imgs,N_h*patch_h,N_w*patch_w,data.shape[-1]))
    k = 0  #iter full img
    s = 0  #iter single patch
    while (s<data.shape[0]):
        #recompone one:
        single_recon = np.empty((N_h*patch_h,N_w*patch_w,data.shape[-1]))
        for h in range(N_h):
            for w in range(N_w):
...
def paint_border(data,patch_h,patch_w):
    ...
    new_data = np.zeros((data.shape[0],new_img_h,new_img_w,data.shape[-1]))
